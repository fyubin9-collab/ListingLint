...
- 自动识别常见中英文字段名，汇总必填映射状态，并在缺少必填字段时阻止误运行。
...
- 按错误/警告筛选问题，按 SKU 与源表行聚合，并突出显示问题原因和修改建议。
- 定位后持续显示当前问题，可切换上一条/下一条，并导出三工作表 Excel 报告。
- 提供约 2 分钟的内嵌功能导览，自动装入错误示例并逐步介绍完整质检流程。
...
若要验证真实上传链路，可在页面下载“示例 CSV”和“示例图片 ZIP”，重新上传后运行质检。
“字段与规则”区域还可下载 JSON 规则示例；复制后修改规则编号、参数和字段别名，再导入即可用于团队口径。
...
账号、云端保存、平台在线接口、远程图片下载、自动修改原表、AI 改写、官方多平台模板、团队权限与批量 API 均不在 v0.1.4 范围内。
